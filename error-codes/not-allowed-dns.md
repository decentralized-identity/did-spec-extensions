# Extension Error Code: `notAllowedDns`
...
The error code is `notAllowedDns`.

## Definition

This DID Resolution error code is used to indicate that a security policy has been triggered, and the DID document does not contain a verifiable link to a domain name.
...
# DID Resolution Option: `checkDns`
...
* `error`: A DID Resolution Result with error `notAllowedDns` is returned if the security policy is triggered.
